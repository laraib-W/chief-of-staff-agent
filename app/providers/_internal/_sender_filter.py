...
def _llm_check_sender(
    config: Config, email: RawEmail
) -> Literal["llm_trusted", "llm_flagged"]:
    try:
        verdict = llm.complete(config, _build_prompt(email), max_tokens=10)
...
def apply_sender_filter(emails: list[RawEmail], config: Config) -> list[RawEmail]:
    """Tag every email's sender_tier. Never removes an email; enforces the LLM cap."""
    cap = config.gmail.max_unknown_sender_llm_calls
    llm_calls_made = 0
    tagged: list[RawEmail] = []

    for email in emails:
        address = _extract_address(email.sender)
        tier = classify_sender(address, config.gmail)

        if tier == "unknown":
            if llm_calls_made < cap:
                llm_calls_made += 1
                tier = _llm_check_sender(config, email)
            else:
                tier = "unchecked"
